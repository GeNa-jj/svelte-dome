<script lang="ts">
  ...
  const getCurrentTimestamp = (): string => {
    return new Date().toLocaleString('en-US', { hour: 'numeric', minute: 'numeric', hour12: true }).split(' ').join('').toLowerCase();
  }
  ...
  const addMessage = (name?: string, message?: string, host:boolean = true): void => {
    ...
    saveMessage(messageList, newMessage, currentPersonId, !isReading, name)
  ...
  // 保存短信
	const saveMessage = (messageList, newMessage, id, isAddUnread = true, name): void => {
		...
		<footer class="border-t border-surface-500/30 p-4">
			<section>
				<div class="input-group input-group-divider grid-cols-[auto_1fr_auto] rounded-container-token">
					...
					></textarea>
					<button disabled="{!currentMessage}" class={`iconfont icon-fasong ${currentMessage ? 'variant-filled-primary' : 'input-group-shim'}`} on:click={addMessage}>
					</button>
				</div>
			</section>
		</footer>
...
<div on:click={onToast}>
	<Toast width="w-full max-w-[640px] opacity-95" position="t" />
</div>
