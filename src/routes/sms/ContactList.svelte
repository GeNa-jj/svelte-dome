					...
					{#each people as person , i}
						...
						>
							<div class="relative inline-block">
								{#if person.unread}
									<span class="badge-icon variant-filled-warning absolute -top-1 -right-1 z-10">{person.unread}</span>
								{/if}
								<Avatar class="avatar" initials="{person.name.slice(0, 1)}" background="bg-primary-600" width="w-10" />
							</div>

							<div class="flex-1 text-start overflow-hidden border-b border-surface-500/30 min-h-[74px]" style="padding-bottom: 9px">
								<div class="flex justify-between">
									<span>{person.name}</span>
									<span class="text-primary-800 font-size-12">{person.lastMessage.timestamp}</span>
								</div>
								<p class="text-primary-800 ellipsis-2-lines font-size-12">{person.lastMessage.message}</p>
							</div>
						</button>
					{/each}
				</div>
